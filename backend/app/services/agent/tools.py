...
# Same threshold and reasoning asrag_service — kept here since
...
def search_docs(query: str) -> str:
    """Search Cobalt Loop's documentation and past support tickets for information relevent to the user's question
    ...
    try:
        ...
        title_by_id = _tool_by_chunk_id(db, [r["chunk_id"] for r in results])
        ...
        return json.dumps({"found": True, "results": excerpts})
    finally:
        db.close()
...
def check_ticket_status(subject_keyword: str) -> str:
    ...
    Use this when the user asks about the status of a specific issue or ticket they've raised, rather than a general question.
    """
    db = get_session()
    try:
        ticket = (
...
def get_service_health() -> str:
    ...
    (API, webhooks, dashboard, workflow engine)Use this when the user
    asks if something is down, degraded, or experiencing an outage."""
